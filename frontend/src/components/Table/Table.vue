        ...
        <tr v-for="(item, idx) in items" :key="idx">
          <td v-for="(key, idy) in colKeys" :key="idy">
            <slot :name="key" v-bind="{ item }">
              <div
                v-if="key === 'group_name' && headers[0].isStudents == 'true'"
              >
                <a @click.prevent="filter(item['group_id'])" href="#">{{
                  item[key]
                }}</a>
              </div>
              <div v-else>
...
import { filter } from "@/store/students/selectors";
export default {
  name: "Table",
  props: {
    items: Array,
  ...
  setup() {
    ...
    return {
      filter(filter_id) {
        ...
        router.push({ name: "StudentsSorted", params: { filter_id } });
      },
    };
  },
};
